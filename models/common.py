...
#C3 improve
from models.convnetx import Block

class C3_ConvNeXt2(C3):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__(c1, c2)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(Block(c_) for _ in range(n)))
...
class Ghost_CoordConv(nn.Module):

    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1,group=1,act=True):
        super().__init__()
        ...
        self.cv1 = Conv(in_channels, c_, k=kernel_size, s=stride, p=None, g=group, act=act)
        self.cv2 = Conv(c_, c_, 5, 1, None, c_, act=act)
...
class GhostBottleneckSim(nn.Module):
    ...
    def forward(self, x):
        return self.conv(x) + self.attention(self.shortcut(x))

class C3GhostSimAM(C3):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(GhostBottleneckSim(c_, c_) for _ in range(n)))
...
class GhostBottleneckSE(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1):  # ch_in, ch_out, kernel, stride
        ...
        self.shortcut = nn.Sequential(DWConv(c1, c1, k, s, act=False), Conv(c1, c2, 1, 1,
                                                                            act=False)) if s == 2 else nn.Identity()
        self.attention = se_block(in_channel=c2)
...
class GhostBottleneckECA(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1):  # ch_in, ch_out, kernel, stride
        ...
        self.shortcut = nn.Sequential(DWConv(c1, c1, k, s, act=False), Conv(c1, c2, 1, 1,
                                                                            act=False)) if s == 2 else nn.Identity()
        self.attention = eca_layer(channel=c2)
...
class C3GhostECA(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(GhostBottleneckECA(c_, c_) for _ in range(n)))
...
class GhostBottleneckCBAM(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1):  # ch_in, ch_out, kernel, stride
        ...
        self.shortcut = nn.Sequential(DWConv(c1, c1, k, s, act=False), Conv(c1, c2, 1, 1,
                                                                            act=False)) if s == 2 else nn.Identity()
        self.attention = CBAM(gate_channels=c2)

    def forward(self, x):
        return self.conv(x) + self.attention(self.shortcut(x))

class C3GhostCBAM(C3):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(GhostBottleneckCBAM(c_, c_) for _ in range(n)))
...
class C3SE(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        c_ = int(c2 * e)  # hidden channels
        self.se=se_block(c2)
        self.m = nn.Sequential(*(GhostBottleneckSE(c_, c_, shortcut) for _ in range(n)))
